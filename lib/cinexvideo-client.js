...
/** Free writing assistance for a single field. Uses no credits. */
...
export async function waitForJob(requestId, { onTick, intervalMs = 4000, timeoutMs = 900000 } = {}) {
  ...
  while (Date.now() < deadline) {
    try {
      ...
      onTick?.({ status: 'running', recoverable_error: true });
    }
    await new Promise((resolve) => setTimeout(resolve, intervalMs));
  }
  const error = new Error(
    lastRecoverableError?.message || 'This generation is taking longer than expected. Check back shortly.'
  );
