...
export async function loadRuntimeSafetySignals({
  ...
  const [openReservations, recentReservations, chargebackRows, revenueRows, outstandingRows] =
    await Promise.all([
      selectRowsFn(
        'credit_reservations',
  ...
  // user_account_status tracks moderation/account activation state, not account
  // creation. Its production schema intentionally has no created_at column.
  // Until account age is sourced from trusted Auth metadata, treat it as unknown
  // rather than failing a paid generation request on an invalid database query.
  const accountAgeHours = Infinity;
