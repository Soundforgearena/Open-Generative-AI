...
/**
 * First draft for a field the writer has not started yet. The demo Director has
 * no project data to draw on, so it offers a usable skeleton rather than
 * pretending to know the story.
 */
function draftFromScratch(context) {
  const label = context.fieldLabel ? context.fieldLabel.toLowerCase() : 'this field';
  return baseResult(
    'First draft',
    `A lone figure arrives somewhere they were told never to return. They came to collect one thing and leave. The place has changed, and so has the reason they came.\n\nOpen wide and let the location speak first, then find the face. Keep the light low and practical, and hold the final image one beat longer than feels comfortable.`,
    `Wrote an opening draft for ${label} so there is something concrete to react to.`,
    'A blank page is easier to fight than to fill — react to a draft and your own instinct arrives faster.',
    ['Who is this person to the place?', 'What do they refuse to say out loud?']
  );
}

function generateDirectorSuggestion(action, context = {}) {
  const actions = { draftFromScratch, generateIdeaDirections, expandStory, buildStoryArc, improvePacing, raiseStakes, createCharacter, suggestEnding, improveDialogue, writeNextScene, tightenScene, createVisualDirection, improveScenePurpose };
