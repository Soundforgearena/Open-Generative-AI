...
export async function selectRows(table, filters = {}, select = '*') {
  ...
  if (!response.ok) {
    const error = new Error(`Database read failed for ${table}`);
    error.status = response.status;
    error.code = data?.code;
    console.error('Supabase REST read failed', {
      table,
...
export async function insertRows(table, payload, { upsert = false } = {}) {
  ...
  const data = await response.json().catch(() => null);
  if (!response.ok) {
    console.error('Supabase REST insert failed', {
      table,
      status: response.status,
      code: data?.code,
      message: data?.message,
...
export async function updateRows(table, filters, patch) {
  ...
  const data = await response.json().catch(() => null);
  if (!response.ok) {
    console.error('Supabase REST update failed', {
      table,
      status: response.status,
      code: data?.code,
      message: data?.message,
...
export async function deleteRows(table, filters) {
  ...
  const data = await response.json().catch(() => null);
  if (!response.ok) {
    console.error('Supabase REST delete failed', {
      table,
      status: response.status,
      code: data?.code,
      message: data?.message,
